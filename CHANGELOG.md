...
### 1.2.3 — 2026-09-07
Pins §5's footprint basis. `RESIDENT_BYTES` and `COLD_BYTES` are the **placed span** of the library's prefixed segments — the extent they occupy in a real link, including `ld65`'s fill — not the sum of member object sizes. A sum is exact only where no segment takes fragments from two objects *and* every inter-segment gap is charged separately; neither is guaranteed and both fail silently on a file split or TU reorder. Measured: c64-https read `X25519_RODATA` at 3,808 B by sum against 3,840 B placed, a 32 B `align = $100` gap that would let a `<=` fit assert pass while the segment overflowed; c64-ChaCha20-Poly1305's five literals understated a real link by 39–295 B on the same basis (their #113). A figure is the sum of its segments' placed spans; gaps between two of the library's own segments are placement, not footprint, since the gap depends on the order the consumer places them in and vanishes if the aligned segment goes first. Reordering relocates that padding rather than removing it — the alignment travels with the segment — so under either order the bytes are consumer-owned, by two paragraphs that agree. c64-x25519 measured 59-210 B of such gap in all seven of their profiles (#142), which under this reading is not an under-declaration — but their own reference cfg places the segments adjacently, so the clause now says in terms that a consumer reserving one contiguous region must add that padding itself from the alignments §4 declares. Padding arising from a segment's own alignment against the consumer's placement is likewise excluded: c64-https demonstrated by relink that routing one of their OWN segments out of an area moved `X25519_RODATA`'s pre-segment pad by exactly one page while the segment's size was unchanged at `$F00`, so it is a consumer property the archive cannot know. Charging it worst-case would have mandated up to `align - 1` of over-declaration per aligned segment — the slack this release declines to bound — and §4 already requires the alignment itself to be declared, from which a consumer derives its own exact pad.
