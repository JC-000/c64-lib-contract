...
### 1.2.3 — 2026-09-07
Pins §5's footprint basis. Each figure is the **placed span** of the segments it covers — the extent they occupy in a real link, including internal alignment fill — not the sum of member object sizes, which is exact only under two conditions neither guaranteed nor required and both silently lost on a file split. Gaps *between* the library's segments and padding *before* them are excluded: both are `(-previous_end) mod alignment`, set by the consumer's placement and ordering, so neither is a library property. Reordering relocates such padding and can **multiply** it — measured at c64-x25519, placing `DATA` first yields 117 B of pre-segment fill plus a second 198 B gap, 315 B against 59 B as shipped, because an empty aligned segment still advances `ld65`'s placement cursor — and a consumer reserving one contiguous region pays both and must take them from its own link map, since these two aggregates cannot express them.

Measured across all five libraries and both consumers before ruling. **All five move**: c64-x25519 and c64-mlkem sum object sizes with no fill term; c64-nist-curves charges 255 B of pre-segment fill per aligned segment and c64-ChaCha20-Poly1305 a per-segment-start charge, both of which this release forbids; c64-polyval measures a contiguous extent, which the sum-of-spans rule narrows. The disagreement is live and in the unsafe direction: c64-wireguard measured `LIB_CHACHA20_POLY1305_CODE` at 8,056 B by object sum against 8,448 B placed, so a library honestly declaring the sum fails their conformant `>=` assert by 392 B; c64-https read `X25519_RODATA` at 3,808 against 3,840 placed, a 32 B `align = $100` gap that would let a `<=` fit assert pass while the segment overflowed.

**A numeric limit on over-declaration was drafted and dropped**, on those same measurements. c64-https, the fleet's only `<=` fit assert, carries 7,334 B of slack against a region with ~200 B of real headroom, because §5 compares a library against a whole region holding the consumer's own code too — a 1024 B over-declaration is invisible there, and the 27,000 B failure the idea was modelled on was a §6.4 per-variant regression. c64-wireguard's 8,921 B is a mix of deliberate superset and one unmodelled configuration axis, not rounding. nist-curves (twelve archives, 98–596 B), polyval (six arms) and CCP were all conformant to any such limit already, so nothing broke without it. It was also the wrong shape twice: against object-size sums a flat 1024 B penalises a library for page-aligning a constant-time segment, and a percentage limit fails polyval's `-compact.a` at 57.5%, which is 256-byte round-up on a 325 B archive.
