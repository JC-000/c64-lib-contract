...
| **§13** | Network backend ABI | Every network backend is source in its consumer's own tree, compiled by that consumer's Makefile into its own binary — never an artifact built by one contract party and linked by another — so there are not two independently-built artifacts (prong 1), and a violation is visible from inside the owning repository's build (prong 2). The two consumers' backend copies have already diverged. | The consumer repositories' own `net_abi.inc` / `net_families.inc` / `net_caps.inc` headers, which are the only place those names are defined or consumed; the v0.17.1 text for the prose |
