...
**v0.10.3 draft (2026-08-15).** The [#76 restructuring](https://github.com/JC-000/c64-lib-contract/issues/76) is **complete**: §6 is the seven-clause build-and-consume chapter (defines-forwarding, `lib-app-owned`, per-variant manifests, the name surface with the §2 ZP prefix registry, footprint asserts, declared non-segment reservations), the document reads core (§1–§7) → domain (§8 crypto, §13 network) → meta, and every version since v0.4.1 is tagged and citable. All four adopters (`c64-nist-curves` v0.10.2, `c64-polyval` v0.6.1, `c64-x25519` v0.11.1, `c64-ChaCha20-Poly1305` v0.9.0) are **conformant at their current tags, verified at the refs** — see [adopters.md](adopters.md) for the per-section table with per-release verification records, [consumers.md](consumers.md) for the two consumer integration rows, and [SPEC.md §12](SPEC.md#12-changelog) for the version history. The composed `c64-https` pair links collision-free at tags. Remaining before a v1.0 declaration: the §6.5 bare-name deprecation windows (which expire at each library's next MAJOR) and the gate discussion on whether 1.0 precedes or follows those expiries.
