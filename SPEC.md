...
; UDP consumers only (§13.3): the backend's net_caps.inc is mandatory, and buffers are sized from it, never from adapter internals
...
A UDP-family backend that ships no `net_caps.inc` is non-conformant from v0.12.0: the `.include` fails the consumer's build, which is the intended failure — loud at assembly, not a stale local constant that fails as an AEAD error on hardware.
