...
**The basis is the placed span.** Both figures measure the extent the library's `LIB_<X>_`-prefixed segments (§4) occupy in a real link — including the fill `ld65` inserts within and before them — **not** the sum of member object sizes. An object-size sum is exact only where no segment takes fragments from two objects *and* every inter-segment gap is charged separately: neither is guaranteed, both stop holding silently on a file split or a translation-unit reorder, and a figure derived from the sum penalises a library that page-aligns a segment to hold a constant-time invariant. Where the fill depends on the consumer's own preceding code, charge the worst case.
