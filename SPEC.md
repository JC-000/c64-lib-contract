...
| `$8A` | `UCI_ERR_LONG_READ` | The SOCKET_READ response header claimed more bytes than were requested **and was not the `$FFFF` no-data sentinel**. **`$FFFF` is the firmware's no-data sentinel on BOTH transports** (measured: every idle UDP poll across 11 sizes returns header `$FFFF`, `rx_len` 0; the first idle TCP polls of a TLS session, waiting for ServerHello, return `$FFFF` against a 512 B request) **and MUST be excluded before any over-claim test** — both fleet adapters independently misfiled it as an over-claim, one for four days as a documented "firmware quirk", the other aborting every handshake on its first idle poll. Disposition is by family: a datagram adapter drops the packet (the firmware discards the excess, GideonZ/1541ultimate#802, so it is unrecoverable) and emits this code; a stream adapter caps its copy at the request (the remainder stays queued, recoverable), continues, and records `$8B` instead. The code cannot distinguish an oversized peer datagram from a header the device fabricated; do not attribute it to peer MTU policy when debugging. c64-wireguard builds before the `$FFFF` exclusion emitted this code on idle polls, meaning "nothing pending". Still unmeasured: the header on an oversized UDP datagram, and the TCP partial-fill case; a stream consumer's copy bound is safe only on the so-far-unrefuted assumption that a non-sentinel header never understates delivered bytes. | terminal (C=1) | c64-wireguard — **in flight**: published `master` still emits this as `$88` (PR #62, 2026-08-20); the move to `$8A` is [c64-wireguard#67](https://github.com/JC-000/c64-wireguard/pull/67), which vacates `$88`. |
| `$8B` | `UCI_ERR_BAD_READ_HDR` | Stream-family counterpart of `$8A`: the header claimed more bytes than were requested and was not the `$FFFF` no-data sentinel, on a byte stream — the adapter caps its copy at the request (the remainder stays queued; nothing is lost) and returns C=0 with this code left as a breadcrumb, so a post-mortem can tell an undocumented header from a normal read. Same observation as `$8A`, different disposition: a stream adapter never emits `$8A`, a datagram adapter never emits `$8B`. | **advisory** (C=0) | c64-https — **in flight** ([c64-https#143](https://github.com/JC-000/c64-https/pull/143)) |

`$FFFF` is called out in the `$8A` row because it is the trap both adapters fell into independently: an empty read, reported as a framing violation. The `$8A` row is worded by disposition rather than by transport on purpose: the same device event is recoverable on a stream and terminal on a datagram, and a table that said only "claimed more than requested" was about to give the byte two operational meanings in two UCI-family adapters — the `$88` failure one row down, caught by the cross-repo view a second time inside one release cycle. The table earns its keep by forcing that question before a byte is emitted. The ip65 codes name the adapter entry point that failed rather than a driver cause, because the ip65 driver reports only a carry. Not every backend emits every code in its family's table (`c64-https`'s UCI adapter reserves `$8A` without emitting it yet); a consumer decoding the byte uses the table, not the emitting adapter's header.
