...
**DMA completion is confirmed, and a post-execute settle is observed, before the next REU register access (v0.13.0, normative).** After every REU execute (`sta $DF01` with the execute bit), and **before the next access to any REU register**, the implementation MUST (a) read `$DF00` and confirm bit 6 (end-of-block) is set, spinning — bounded, §13.4-style — if it is not, and (b) leave a **post-execute settle** that meets the empirically-bracketed floor across all supported devices and clocks, §13.6's standard. The two are separate obligations for separate hazards: (a) covers a device on which the CPU is *not* held for the transfer — not yet observed, but the cheapest insurance there is; (b) covers the hazard that *has* been observed, the REU's post-transfer restore outlasting a turbo CPU so that the next register write lands on a busy controller. Neither MAY be assumed from the REU's DMA line having halted the CPU. Measured floor so far: **≥ 49 cycles at 48 MHz on a U64E, fw 3.15** (the status read itself costs that much, so a read-once implementation meets it at 48 MHz *by accident*); **unbracketed at 64 MHz** (C64 Ultimate) — an adopter supporting 64 MHz MUST bracket it there before claiming conformance at that clock, and §13.6's fence (~63 µs) is a safe over-estimate in the meantime. Reading `$DF00` clears bits 5–7, so **read it once per iteration and test what that read captured — never re-read to test a second bit.** Two capture forms are conformant and the choice has a consequence worth making deliberately: loading into a register (`lda $DF00`) keeps every bit available, so bit 5 (verify fault, an error) can be recorded alongside the bit-6 confirm; `bit $DF00` instead copies bit 6 straight into V and bit 7 into N without touching the accumulator, which costs nothing in register pressure at a hot site but **discards bit 5 irrecoverably** — the read has already cleared it and no register holds it. An implementation that wants the verify-fault signal MUST use a form that captures the whole byte. Both adopters of this clause chose differently and both are conformant (c64-x25519 records bit 5; c64-nist-curves documents that it cannot at its inlined sites, since it issues no VERIFY command and so cannot raise it). This applies on **both** paths: the row fetch (`reu_fetch_mul_row`) and every store in the table build (`reu_mul_tables_init`) — a table written under the same race is silently wrong before a single multiply runs. On expiry the caller's error path applies; the primitive has no error channel of its own, so an adopter SHOULD surface a bounded-spin failure the same way it surfaces a missing REU at init.

**A settle met by intervening code is conformant, and is the easiest one to lose (non-normative).** Obligation (b) says nothing about *how* the cycles are spent, so a site whose next REU register access is already far enough away — the next row fetch across an accumulate body, say — satisfies it structurally with no added instruction, which is the cheapest correct implementation available and worth taking. The hazard is that the constraint then lives nowhere in the code: the distance is a property of unrelated instructions that no one editing them can see, the obvious optimisation is to shorten exactly that path, and no emulator reproduces the failure (bit 6 is set on the first read under emulation, and there is no restore window to overrun). An adopter relying on a structural settle SHOULD therefore state the required distance where the shortening would happen, not only at the execute site, and SHOULD prefer an assembly-time assertion over a comment wherever the distance is expressible as one. The same reasoning applies to a settle satisfied by the confirm read's own I/O cost: it holds at 48 MHz — the measurement above is what makes it hold — but nothing about it is self-evident at another clock, so the clock it is claimed for belongs in an assertion rather than in prose.

**Failure mode this prevents (measured, 2026-08-27, [#144](https://github.com/JC-000/c64-lib-contract/issues/144)).** Every adopter's `reu_mul` path was `sta $DF01` → immediate use of the transferred bytes or the next register write, with no status read: correct by hardware design on a real 17xx REU, and correct on the U64E's FPGA REU through fw 3.14d. On fw 3.15 (`update_v3.15-74-g6b5ffc21`, FPGA 123, core 1.4E) at 48 MHz turbo the same build fails every Noise handshake with an AEAD `InvalidTag` — wrong field products presenting as an authentication failure two protocol layers up — while the no-REU build passes and the **same REU build passes at 1 MHz** (c64-wireguard#69: 779 s handshake). With the `$DF00` confirmation at all 11 execute sites the REU build passes 9/9 at 48 MHz (53 s) — measured on a local experimental branch of the `c64-x25519` primitive linked into c64-wireguard; **no published tree carries the fix yet**: c64-wireguard `master` and #68 ship the unfixed primitive (`REU=0` is their hardware recommendation until it lands), and the canonical fix belongs in `c64-x25519`, which owns `reu_mul` under this clause — tracked at [c64-x25519#115](https://github.com/JC-000/c64-x25519/issues/115), with `c64-nist-curves` re-verifying against it. The mechanism, as far as measured: bit 6 was **already set on the first read at every one of 19 416 calls** (max poll iterations 0, bit 5 never, 0 timeouts), so the CPU *is* stalled for the transfer itself; what the old code broke is the **next REU register write** landing too soon after the CPU resumes — consistent with 3.15's post-transfer autoload/restore outlasting the CPU at turbo — and the status read plus its ~50 cycles is what separates them. Split (same device, same 11 sites): a **delay-only** variant — no `$DF00` read, ~49 cycles after each execute — also passes 9/9 at 48 MHz, so the status read is *not* load-bearing and the mechanism is a settle; that ~1 µs is far below §13.6's ~63 µs C64U floor and says nothing about 64 MHz, where the same cycle count is shorter — hence the settle is a bracketed floor, not a fixed number, and the read-once form must not be mistaken for self-timing. This is the first §8.2 record scoped by firmware and clock; adopters' REU rows in adopters.md are verified on 3.14d until re-verified with this clause on 3.15 ([c64-x25519#115](https://github.com/JC-000/c64-x25519/issues/115)).
...
> Numbering note (v0.10.1): section numbers are **stable identifiers, not positions** — §2/§3/§4/§8.x are referenced by name from adopter and consumer cfgs, docs, and commit messages (measured at the v0.9.x wave tags: 900+ §8.x references fleet-wide), so no section is ever renumbered. §13 was historically appended after the §12 changelog to preserve that invariant; as of v0.10.1 the *text* is ordered for reading — core, then domain chapters, then meta — while every number stays fixed. New chapters take the **next number not already allocated in this document**, placed with their kind — deliberately not a literal, because a sentence naming the next free number goes stale the moment someone takes it and then misdirects the following author into claiming an occupied one.
...
| `$40-$7F` | ip65-family backends — `$41-$45` are c64-https's TCP/DNS surface; `$46-$47` allocated v0.14.0 are the fleet's first ip65 UDP codes. `$40` has never been allocated |
| `$80-$BF` | UCI-family backends — the pre-existing `UCI_ERR_*` values (`$81-$89`) are grandfathered unchanged; `$8A` was allocated to resolve the collision below; `$8B` is the first code allocated under the table rule; `$8C` the second, and the first with an observable trigger; `$8D` allocated v0.13.0 |
...
| `$41` | `NET_ERR_IP65_INIT` | `ip65_init` failed (no RR-Net / cs8900a) | terminal (C=1) | c64-https; **second emitter** c64-wireguard's ip65 backend, v0.14.0 ([#148](https://github.com/JC-000/c64-lib-contract/issues/148)) |
| `$42` | `NET_ERR_IP65_DHCP` | `ip65_dhcp_init` failed (no lease) | terminal (C=1) | c64-https; **second emitter** c64-wireguard's ip65 backend, v0.14.0 ([#148](https://github.com/JC-000/c64-lib-contract/issues/148)) |
...
| `$46` | `NET_ERR_IP65_UDP_LISTEN` | `udp_add_listener` refused the bind: ip65's listener table is full (`udp_cbmax = 4`) or the port is already handled. The two causes deliberately share one code — ip65 collapses them itself, both reaching the same tail with the same `ip65_error`, so an adapter cannot separate them without duplicating the driver's own bookkeeping. | terminal (C=1) | c64-wireguard ([#148](https://github.com/JC-000/c64-lib-contract/issues/148), filed before use) |
| `$47` | `NET_ERR_IP65_UDP_SEND` | `udp_send` failed — the transmit was rejected below IP (`udp_send` → `udp_send_internal` → `ip_send`). Separate from `$45`, which is `ip65_tcp_send`: the ip65 codes name the failing **adapter entry point**, and one byte covering two transports would leave a post-mortem unable to say which one failed. | terminal (C=1) | c64-wireguard ([#148](https://github.com/JC-000/c64-lib-contract/issues/148), filed before use) |
...
| `$8D` | `UCI_ERR_OPEN_REFUSED` | The firmware **refused** a socket open: ERROR bit clear, no socket id on RESP_DATA, but a named refusal on the `$DF1F` STATUS channel (e.g. `85,ERROR OPENING SOCKET` — UDP PCB pool exhausted, `MEMP_NUM_UDP_PCB = 8`, GideonZ/1541ultimate#808). Distinct from `$88` (no id *and no status* — phantom/short read) and `$89` (bounded wait elapsed — wedge): here the device answered, and said no. The adapter captures the STATUS line so the firmware's decimal code is available to the caller; a consumer can surface "pool exhausted, retry later" instead of an opaque timeout. **General rule this row establishes:** a UCI adapter MUST drain STATUS on command completion *before* waiting on RESP_DATA, and a non-`00` status with the ERROR bit clear is a refusal, not a wedge — the same unread-STATUS habit hid `82,PARAMETER(S) OUT OF RANGE` behind the 512-byte misreading (§13.3). | terminal (C=1) | c64-wireguard ([#147](https://github.com/JC-000/c64-lib-contract/issues/147), filed before use; published in c64-wireguard#73, merged 2026-08-27, `master` `e4fc505`) |

`$FFFF` is called out in the `$8A` row because it is the trap both adapters fell into independently: an empty read, reported as a framing violation — and, once, used as a copy count. The portable lesson is not "the firmware over-claims" (nothing measured shows that it does) but **exclude the sentinel before any length arithmetic, and never let a device-supplied count be the only bound on a store loop**. The `$8A` row is worded by disposition rather than by transport on purpose: the same device event is recoverable on a stream and terminal on a datagram, and a table that said only "claimed more than requested" was about to give the byte two operational meanings in two UCI-family adapters — the `$88` failure one row down, caught by the cross-repo view a second time inside one release cycle. The table earns its keep by forcing that question before a byte is emitted. The ip65 codes name the adapter entry point that failed rather than a driver cause, because the ip65 driver reports only a carry.

**A backend MUST translate its driver's native error values into its own family range, never forward them (v0.14.0).** This is a range rule, not a matter of taste. ip65's own `ip65_error` namespace runs `$80-$A1` (`inc/error.inc`) and therefore lies **entirely inside this table's UCI range**: an ip65 backend that passed the driver byte through would emit `$87` for a full listener table — published one section down as `UCI_ERR_SHORT_WRITE` — and `$8A` for a closed connection, already `UCI_ERR_LONG_READ`. That is the `$88` collision a third time, sourced from a vendored driver rather than a sibling adapter, and it would breach the family-range rule above while looking, at the call site, like diligence. The temptation is concrete rather than theoretical: most ip65 entry points return only a carry, so there is nothing to forward — but `udp_add_listener` *does* set a specific cause (`IP65_ERROR_LISTENER_NOT_AVAILABLE`), which makes the one path that offers a driver cause also the one whose value collides. Any family wrapping a driver that has its own error vocabulary inherits this rule; the family range is a namespace this contract hands out, and a driver's namespace was never in it.

**The over-length send refusal is a contract-generic condition with a grandfathered family spelling (recorded v0.14.0, not allocated).** `$8C UCI_ERR_SEND_TOO_LONG` names a condition §13.3 states family-independently — the datagram exceeds the backend's published `NET_UDP_SEND_MAX` — so nothing about it is UCI-specific. `$8C` stays as it is; published values are never reassigned. But a second UDP family that needs the same runtime pre-check MUST NOT mint a parallel family byte for it: it requests a `$01-$3F` contract-generic code (the `$01` precedent), and this table then records the two as one condition with a grandfathered UCI spelling. This is recorded rather than allocated because no second backend can reach the check yet — c64-wireguard's ip65 backend publishes `NET_UDP_SEND_MAX = NET_UDP_RECV_MAX = 1472`, both bound by the IP MTU, and pins its MTU below that with the §13.8 asserts, so the condition is caught at assembly time and the runtime path is unreachable there. Settling it now costs a paragraph; settling it at the third family would cost another collision. Not every backend emits every code in its family's table (`c64-https`'s UCI adapter reserves `$8A` without emitting it yet); a consumer decoding the byte uses the table, not the emitting adapter's header.
...
- *Send, U64E fw 3.15* (same measurement, [#141](https://github.com/JC-000/c64-lib-contract/issues/141)). One `SOCKET_WRITE` is one `lwip_send` on a `SOCK_DGRAM` socket — one datagram — and there is no command-direction multi-block, so the outbound ceiling is hard: the command buffer is 896 B but `command_protocol.vhd` saturates `command_length` at 895, and 895 − 3 (target, command, socket id) = **892** (`860 → [860]`, `892 → [892]`, `893` splits). An adapter that chunked at 800 B put `[800]`, `[800, 200]`, `[800, 652]` on the wire for 900/1000/1452 B sends with C=0 and `net_last_error = $00` — silent in every case, which is why `$8C` exists and why the refuse-before-write rule is a MUST. A chunked-write command would lift it: GideonZ/1541ultimate#802 (closed; its receive half shipped as #806) spun the send side out to **#807**, where both candidate designs — `WRITE_SOCKET_MORE` and an announced-total-length write — were built into one firmware image and reach **1472 byte-exact** on a U64 Elite (3.15, FPGA 123, core 1.4E), the announced-total form — proposed upstream as `WRITE_SOCKET_CHUNK` — being preferred because an abandoned partial sequence flushes less. Under the minimum-across-supported-firmware rule a UCI backend keeps `NET_UDP_SEND_MAX = 892` until a firmware carrying #807 is its floor; then it raises the header and every consumer's `.assert` re-checks — and re-measures its *part* size rather than reusing 892: #807's header is five bytes (`$03 $12 <handle> <total lo> <total hi>`) against `WRITE_SOCKET`'s three, so a part carries at most **890** B; a client that hands over 892 per part delivers two bytes fewer than it thinks, and the firmware's empty-reply convention is the only thing that makes the shortfall visible (measured upstream: 880 + 1120 parts of a 2000 B total held 1770). Two forward notes for whoever implements it, non-normative until #807 ratifies: the *part* size is an adapter-internal constant (`UCI_WRITE_PART_MAX`-shaped, alongside `UCI_DATA_QUEUE_MAX`), **not** a `net_caps.inc` value — `NET_UDP_SEND_MAX` remains the one consumer-facing guarantee (the whole-datagram ceiling, 1472 under #807), and the contract does not add a `NET_UDP_SEND_PART_MAX`; and the announced total MUST be validated against `NET_UDP_SEND_MAX` on the *first* part, before anything is handed over, because an over-size total accumulates in the firmware and fails only at the final `lwip_send` (`12,SEND ERROR:5`, `IP_FRAG = 0`) — which is the `$8C` refuse-before-write rule applied to a multi-part write — at which point a RAM-bound consumer's own cap (below) is what binds. #807 also confirms the 2-byte written count is specified (UCI Network Target Command Summary v1.1 §2.10), so an adapter's short-write check (`$87`) is per spec, not defensive.
...
**Failure mode this prevents (measured, 2026-08-27, [#145](https://github.com/JC-000/c64-lib-contract/issues/145)).** Both UCI adapters in the fleet implemented the wait shape below and neither ever wrote the TOD. IRQ-hooked sampling on a U64E in the c64-wireguard#58 stalled state: 207 reads over 3 s, TOD `00:00:00.0` throughout, hour byte at its reset value. Every "wall-clock bounded" wait had therefore been **unbounded on real hardware since the day it was written** — no live run in either project's history had ever produced `$89 UCI_ERR_WAIT_TIMEOUT` — and a firmware command that did not complete hung the C64 forever. Proof of mechanism: a one-shot `sta $DC08` from the hook made the hung `net_udp_send` expire within 2 s and return C=1. The bounded-wait conversion had been "tested" only by runs that ended because the device answered, never because the bound fired; a §13.8 conformance test for this clause MUST include a case where the device does *not* answer (e.g. a wait on a socket that is never written) and MUST observe the timeout code. Note what that test proves: on VICE the TOD runs from reset, so an emulator run exercises the *bound* but not the *start* — the start clause is provable only on hardware (or an emulator configured with the TOD stopped at reset), and the verification record SHOULD say which. Reference implementation of this clause: c64-wireguard#68's `uci_tod_start` (merged 2026-08-27, `master` `e4fc505`), verified on a U64E in the #58 stalled state — `net_init` C=0, and a `net_udp_send` that previously hung forever returned C=1 in 1.5 s.
...
| `c64-wireguard` ([#48](https://github.com/JC-000/c64-wireguard/issues/48)) | **Largely landed in [c64-wireguard#67](https://github.com/JC-000/c64-wireguard/pull/67)** (merged 2026-08-27, `8088c43`; device-verified on U64E fw 3.14d and 3.15): the rename, carry semantics, un-export, `net_udp_dest_*`, the `$8A` move, bounded waits, the fence floor, the phantom-socket guard, `net_udp_close`. **Then [c64-wireguard#68](https://github.com/JC-000/c64-wireguard/pull/68)** (merged 2026-08-27, `e4fc505`): the §13.3 `net_caps.inc` reference implementation (baseline fw 3.15+: `NET_UDP_RECV_MAX = 1472`, `NET_UDP_SEND_MAX = 892`, `WG_MTU = 860`, 1472 B receive and 892 B send hardware-verified), the §13.8 capability asserts, the `$8C` pre-check, the 16-bit length path, and the §13.4 `uci_tod_start`; #73 (same day) added `$8D` and close-on-teardown. Still deferred: `net_manifest.s` + `net_abi_asserts` (ip65 backend exports neither `net_local_ip` nor `net_last_error`, so a core-family assert would link green over an absent error channel — fix the backend first). **Unblocked v0.14.0**: `$46`/`$47` complete the ip65 UDP vocabulary that backend was missing, so the error channel, then the manifest and asserts, can land ([#148](https://github.com/JC-000/c64-lib-contract/issues/148)). Its REU build is known-failing at 48 MHz on fw 3.15 until the §8.2 fix lands in `c64-x25519` (c64-wireguard#69). Original item list, kept for the record: rename `net_dhcp` → `net_dhcp_acquire`; retire `net_poll`'s "C=0 = packet" meaning (§13.2); un-export `net_save_zp`/`net_restore_zp` (§13.5); replace `wg_peer_ip`/`wg_peer_port` reads in the adapter with `net_udp_dest_ip`/`net_udp_dest_port` — **not a mechanical rename outside the adapter**: any host-side tool or test that calls `net_udp_send` directly becomes responsible for staging those cells, and one that stages only `wg_peer_*` **fails silently**: it sends to `0.0.0.0` with carry clear and `net_last_error` zero, i.e. it looks exactly like a working send (measured; it got past a 22-suite gate and four live tools at once and cost two device runs on hardware with ~5 good cycles per power-up — the dangerous failures are the ones that present as success, as with §13.3's silent truncation); rename send globals to `net_udp_send_ptr`/`net_udp_send_len`; **resync the UCI adapter with `c64-https`'s hardened one** (bounded waits §13.4, fence floor §13.6, phantom-socket detection — its `uci_errors.inc` predates `$88`/`$89`); ship `net_manifest.s` + `net_abi_asserts`. |
...
A public entry point MUST terminate on every input of its parameter type, **or** document the domain over which it terminates wherever that domain is narrower than the type. Where the non-terminating inputs can be excluded cheaply at the entry, the library SHOULD do so and return an error rather than rely on every caller having read the domain, **signalled the way its other entry points already signal failure**. This clause does not add a convention and does not prefer one: §13.2 fixes carry for the network chapter, but §8's territory has no fleet-wide equivalent and the crypto adopters use `A` (c64-polyval `A=1`/`Z=0`; c64-ChaCha20-Poly1305 `A=$ff`). An entry point that has never had a failure path acquires one here, which is a real ABI addition rather than a documentation exercise — cost it as such.
...
Where the narrower domain is a bound the consumer must respect — a maximum length, a ceiling — the library SHOULD publish it in a consumer-includable header as **a symbol a consumer can reference**: an equate where the bound fits one, an exported label where it does not. The distinction is not pedantry — a ca65 equate holds at most 32 bits, so a 256- or 384-bit modulus (`fp_mod_add`'s operand bound) cannot be an equate at all, and requiring one would leave a bignum library non-conformant to a SHOULD it has no way to satisfy. A consumer SHOULD reference the published symbol rather than hard-code or re-derive the value.

**Where the domain is a relation rather than a constant, this clause does not apply and §14.1 alone discharges the obligation.** `c64-ChaCha20-Poly1305`'s real restriction is `aead_data_ptr + aead_data_len <= $10000` — a relation over two caller-supplied values that no scalar expresses, and that a consumer whose buffer address is computed at runtime cannot assert at assembly time either. Publishing a scalar there is worse than publishing nothing: every candidate in circulation for that library was either vacuous (`$FFFF`, the field width) or wrong in §14.1's sense (`1500`, an MTU inherited from an unrelated consumer; `3840`, the size of its own fuzz harness's scratch window), and a consumer placing a buffer at `$FF00` satisfies `1500` and still wraps into zero page. Document the relation in the terms the implementation tests, guard it at the entry, and publish only what genuinely is constant.

This is §13.3's `net_caps.inc` pattern arriving independently in the crypto chapters: [c64-polyval#76](https://github.com/JC-000/c64-polyval/pull/76) published `gcmsiv_max_pt_len = 64` with buffers sized from it and both entry points rejecting above it, before any clause asked. §13.3 exists because a number living only in prose gets re-derived wrongly by each consumer, and nothing about that lesson stops at the chapter boundary. Publishing it also makes the domain testable, which is what **§15.1** asks of any check offered as evidence: once the bound is a named constant, "feed the bound, then feed bound+1" is a check whose failure mode is obvious, which is what [c64-polyval#76](https://github.com/JC-000/c64-polyval/pull/76) then ran (`pt_len` ∈ {65,128,255} probes, rejection with ciphertext, tag and expanded key untouched).
...
- **No per-entry-point audit burden.** Of `c64-nist-curves`'s public surface exactly one routine has an input-dependent termination condition — everything else is a fixed-trip loop or a fixed-width scalar walk — and `c64-ChaCha20-Poly1305` has none at all across 37 exported entry points and 29 runtime loops: 23 are **fixed-trip** (a constant iteration count no input reaches — strictly stronger than a countdown, since it cannot be input-dependent even in principle), 5 are length-bounded behind a true entry guard, and one is neither. That last one is worth naming, because it is a fourth bound shape alongside the three above: `poly_ripple`'s exit test is `inc`/`bne`, data-dependent on the byte it increments, yet the routine is **total on its whole parameter type** — an index that strictly increases meets a hard ceiling, so every input either exits at once or climbs to it. A loop can be data-dependent in its exit condition and still provably terminate; "input-dependent" and "unbounded" are not the same property. Where that ratio holds, the cost of this section is "audit the loops once, then guard one or two".
...
**Where the cited evidence is an aggregate gate, the obligation applies per check within it, not to the gate as a whole.** An adopter cites `make check-archives`; the property a clause depends on is enforced by one leg of it. Without this scoping an adopter can say truthfully that its ratchet has been demonstrated to fail on the strength of one negative-tested leg while eleven others have never fired — which is the original defect wearing the gate's name.

**A demonstration is scoped to the configuration it was performed in.** Where a check is relied on in a configuration it has not been exercised in — most commonly a **control arm**, which is by construction the configuration the instrument was built least to serve — the demonstration does not carry over, and the check SHOULD be exercised there before its result is treated as evidence.

The motivating case is a hardware probe carrying a negative control, a detector positive control and a 36-check self-test, all green against the mitigated build it was developed on. Pointed at the **unmitigated control** — the one configuration the experiment turned on — it failed in four independent places before producing a single measurement. The fourth is the instructive one: a missing entry point resolved to the trampoline's own entry, so `jsr` on an absent routine presented as a 243-second boot and a **dead-REU timeout**. A tool defect wearing a hardware symptom, in the arm where the result would have been believed.

A published domain bound (**§14.2**) — a ceiling or exclusion emitted as a symbol a consumer can reference, rather than described in prose — composes with this directly: once the edge is a named constant, "feed the bound, then feed bound+1" is a check whose failure mode is obvious.
...
Doc/correction (PATCH): **the 0.16.0 entry's fleet position contradicted §14.2 in its own release.** Two sentences in that entry said `c64-ChaCha20-Poly1305` "owes one stated ceiling" / "owes only a stated ceiling" for `aead_data_len`, while §14.2 as shipped in the same tag says the opposite in normative text: that library's restriction is the relation `aead_data_ptr + aead_data_len <= $10000`, and **"where the domain is a relation rather than a constant, this clause does not apply and §14.1 alone discharges the obligation."** §14.2 names that exact relation and explains why every scalar candidate for it was either vacuous (`$FFFF`, the field width) or wrong (`1500`, an MTU from an unrelated consumer). This is not drift: `git show v0.16.0:SPEC.md` carries both the carve-out and the contradicting sentences, so the entry was self-contradictory on the day it shipped. The carve-out was added during [#161](https://github.com/JC-000/c64-lib-contract/pull/161)'s review and the fleet-position lines were not re-read against it.

The consequence was not cosmetic. An adopter checking its own status reads the changelog's fleet position, not the clause body — so the entry directed the one library §14.2 carves out to go and publish exactly the vacuous scalar that clause calls **worse than publishing nothing**. Reported by that adopter while recording its own conformance ([c64-ChaCha20-Poly1305#99](https://github.com/JC-000/c64-ChaCha20-Poly1305/pull/99)), which stated the conflict neutrally and deferred to §14.2 rather than asserting the contract was wrong — the right handling, and the reason it was caught at all.

Both sentences are corrected in place with an inline marker rather than silently rewritten, since §12 entries are pinned and cited by tag. No clause, code, bit or equate changes; §14 and §15 normative text is untouched. **Process note for future releases:** a "fleet position" line makes a per-adopter claim that a clause elsewhere in the same release may already answer, and the two are written at different times by different reviewers. Re-read every fleet-position sentence against the final clause body before tagging — the changelog is the surface adopters actually check.
...
A second adopter addition, raised on [#144](https://github.com/JC-000/c64-lib-contract/issues/144) and proposed on the PR rather than added on their behalf, scopes the demonstration itself: **a check shown capable of failing in one configuration is not thereby shown in another.** Their evidence is their own and unflattering — a hardware probe with three layers of self-check, all green, that broke in four independent places the first time it met the control arm, one of them presenting a tool defect as a dead device. The control arm is where an instrument is least exercised and most depended on, which is the combination worth naming.

Adopter review of the draft added one sentence that changes what the clause catches: **the obligation applies per check within an aggregate gate, not to the gate as a whole.** The adopter raising it cites `make check-archives` — twelve archives plus legs for §1 identity, §4 placement, §5 value pins, §6.3 knob staleness, §6.5 gated surface, §8.2/§8.3 provider surfaces and a ZP alias audit — and reported that some legs have been negative-tested (knob staleness, the alias audit, the byte-distance settle asserts) while several §5 and §8.3 pins never have. Without the scoping sentence that library could have claimed the gate demonstrated-failing on the strength of one leg. It is the difference between the clause catching that situation and missing it, and it cost one sentence.
...
**Three changes came from adopter review of the draft, and two of them were defects.** §14.1 originally prescribed "carry set" as the rejection convention — which **neither** crypto adopter it cites uses (c64-polyval signals `A=1`/`Z=0`, c64-ChaCha20-Poly1305 `A=$ff`); §13.2's carry rule is scoped to the network chapter and there is no §8 equivalent, so the clause now names no convention and defers to whatever the library already uses. §14.2 originally required an **equate**, which a ca65 equate cannot hold for a 256- or 384-bit modulus, leaving a bignum library non-conformant to a SHOULD it had no way to satisfy; it now asks for a referenceable symbol, equate or exported label. And §14.2 now says explicitly that where the domain is a **relation over caller-supplied values** rather than a constant — `aead_data_ptr + aead_data_len <= $10000` — §14.1 alone discharges it, because every scalar in circulation for that case was either vacuous or wrong in §14.1's own sense. §14.3's loop inventory was also corrected against a real audit: 23 of that library's 29 loops are fixed-trip rather than length countdowns, and one terminates on a data-dependent test while remaining total, which is a fourth bound shape the draft had no name for.
...
Classified MINOR: new normative content, no change to any existing clause, code, bit or equate. **Fleet position at merge:** `c64-polyval` conformant since #76; `c64-nist-curves` conformant at `master` since [#139](https://github.com/JC-000/c64-nist-curves/pull/139) (`92ef7bc`, 2026-08-30) closed #132 and #135, not yet at a tag; `c64-ChaCha20-Poly1305` is conformant via §14.1 alone: its restriction is the relation `aead_data_ptr + aead_data_len <= $10000`, which §14.2 explicitly carves out from the publish-a-bound SHOULD, and it guards both pointers at both entry points ([#98](https://github.com/JC-000/c64-ChaCha20-Poly1305/pull/98)). *(Corrected in 0.17.1 — this sentence and the one above originally said it owed a stated ceiling, contradicting §14.2 in this same release.)* **This section is not retroactive**: an adopter conformant before it landed is not out of conformance the moment it does, and no adopter is out of conformance at a released tag on the day it lands.
...
Clarifying (PATCH): **§8.2's read-once rule now names both conformant capture forms, and records that a structurally-met settle is the easiest one to lose.** Both changes come from the clause meeting two independent implementations on the same day ([c64-x25519#116](https://github.com/JC-000/c64-x25519/pull/116), [c64-nist-curves#131](https://github.com/JC-000/c64-nist-curves/pull/131)) — which is the first time v0.13.0's §8.2 had been written against by anyone.

The read-once sentence said *"read once per iteration into a register and test there"*, which describes `lda $DF00` and accidentally excludes `bit $DF00` — a form that copies bit 6 into V without touching the accumulator and is strictly better at a hot site under register pressure. It was always the intent that the rule forbids the **second read** (which would find bits 5–7 already cleared), not that it mandates a particular instruction, so the wording is corrected to say capture-then-test rather than register-then-test. The trade-off is now stated with it, because it is real and not obvious: `bit` discards bit 5 irrecoverably, so an implementation that wants the verify-fault signal must load the whole byte. The two adopters chose differently, both conformant, and the clause now says so rather than leaving the next reader to assume one of them is wrong.

The second addition is non-normative and is the more useful of the two. Obligation (b) constrains the *interval* before the next REU register access, not how it is spent, so a site with enough unrelated work in between satisfies it for free — correct, and worth taking. But the constraint then exists nowhere a reader can see it: it is a property of instructions that have nothing to do with the REU, the natural optimisation is to shorten precisely that path, and **no emulator reproduces the failure**. That combination — a real constraint held only by a comment, guarded by a test suite structurally blind to it — is exactly the shape that let the original defect survive, and it now recurs one layer up in the fix. The clause therefore asks that a structural settle be documented where the shortening would happen and, where the distance is expressible, asserted rather than described. The same is said of a settle satisfied by the confirm read's own I/O cost: it holds at 48 MHz because the measurement says so, and the clock it is claimed for belongs in an assertion.

No normative obligation changes: the two obligations, the measured floor, the 64 MHz bracket and every allocated code are untouched, and both in-flight adopter implementations remain conformant as written.

### 0.14.0 — 2026-08-28

Normative (MINOR): **§13.2 allocates the first ip65 UDP codes and forbids forwarding a driver's native error values.** Filed before use ([#148](https://github.com/JC-000/c64-lib-contract/issues/148)), the third allocation under the table rule. `$46 NET_ERR_IP65_UDP_LISTEN` and `$47 NET_ERR_IP65_UDP_SEND` exist because **c64-https's ip65 backend implements no UDP at all** — its `$43`/`$44`/`$45` are DNS, TCP-connect and TCP-send — so there was nothing in the fleet to adopt. `$45` was the near miss: reusing it for `udp_send` would have put two entry points behind one byte, against the ip65 convention that a code names the adapter entry point that failed, and left a post-mortem unable to say which transport broke. `$41`/`$42` are recorded as acquiring a second emitter, which is the other half of the same discipline — adopt the existing spelling, do not invent a parallel one.

The codes are not bookkeeping. §13.1 puts `net_local_ip` and `net_last_error` in `NET_FAMILY_CORE`, and c64-wireguard's ip65 backend exports neither, because ip65 reports only a carry and the adapter had no vocabulary to translate it into; so §13.8's `NET_FAMILY_CORE` assert **would have linked green over an absent error channel** — precisely the failure this chapter's assert set exists to prevent. That backend held its manifest and asserts back rather than ship a claim it could not honour, which is the rule working: the gap was declared at the point of non-conformance instead of asserted away. It is also not a corner case — `BACKEND ?= ip65` is the default in that repo's Makefile, so an unqualified `make` produces the backend with no error channel.

The forwarding ban is the sharper half. **ip65's own `ip65_error` namespace (`$80-$A1`) lies entirely inside this table's UCI family range**, so an adapter that passed the driver's byte through would emit `$87` for a full listener table — already `UCI_ERR_SHORT_WRITE` here — and `$8A` for a closed connection, already `UCI_ERR_LONG_READ`: the `$88` collision a third time, this time sourced from a vendored driver rather than a sibling adapter. What makes it worth a MUST rather than a note is where the temptation sits: nearly every ip65 entry point returns a bare carry with nothing to forward, but `udp_add_listener` *does* set a specific cause, so the single path that hands an author a real driver diagnosis is the same path whose value collides — and forwarding it would read like diligence at the call site. Two adapters had already given one byte two meanings by generalising from their own transport; this is the same shape one layer down, caught before emission because the code was filed first.

Also recorded, not allocated: **the over-length send refusal is a contract-generic condition** that happens to have a UCI spelling. §13.3 states the rule family-independently, so a second UDP family needing the runtime pre-check requests a `$01-$3F` code rather than minting a parallel family byte; `$8C` stands, grandfathered. No allocation yet because no second backend can reach the check — the ip65 backend's 1472/1472 caps and §13.8 asserts catch it at assembly time.

Classified MINOR: new normative content (two codes, one MUST), with every previously allocated code, range and bit value unchanged, so no conformant adapter is affected.
...
Normative (MINOR): **§8.2 requires confirming REU DMA completion via `$DF00` bit 6 before the next REU register access**, on both the fetch and the table-build paths ([#144](https://github.com/JC-000/c64-lib-contract/issues/144)). Every adopter assumed the REU halted the CPU for the whole transfer and touched the next register immediately; on U64E fw 3.15 at 48 MHz that produces wrong products — every REU-backed handshake failed its AEAD while the no-REU build and the 1 MHz REU build passed — and the status confirmation at every execute site restores 9/9. Measured: the transfer is complete when the CPU resumes and a delay-only variant passes too, so the hazard is the next register write landing during the REU's post-transfer restore, and the rule is two obligations — confirm bit 6 (insurance for a device that does not hold the CPU) and a §13.6-style bracketed settle floor (≥ 49 cycles at 48 MHz on U64E fw 3.15; open at 64 MHz). Also in this release: **`$8D UCI_ERR_OPEN_REFUSED`** joins the §13.2 table ([#147](https://github.com/JC-000/c64-lib-contract/issues/147), filed before use) — the firmware refusing a socket open on the STATUS channel with the ERROR bit clear, which both adapters had been reporting as a phantom socket or a timeout; its row carries the general rule that STATUS is drained before RESP_DATA is awaited and that a non-`00` status is a refusal, not a wedge. The two clauses in this release share one shape and one cause: a mechanism the hardware used to provide for free, assumed rather than confirmed, that a firmware release stopped providing at turbo.

### 0.12.1 — 2026-08-27

Clarifying (PATCH): **§13.3 says a consumer MAY pin its MTU below the backend's ceilings, and that doing so does not shrink the receive buffer.** Raised by the first §13.3 implementation (c64-wireguard#68): the ip65 backend honestly publishes 1472/1472, and a consumer deriving MTU purely from the backend's pair would size buffers it cannot afford. The consumer folds its own cap into the minimum; the §13.8 asserts stay as upper bounds; `udp_recv_buf` still holds `NET_UDP_RECV_MAX` because the backend delivers up to that whole and the peer is not bound by this consumer's MTU. No wording in §13.8 changes. Also: the §13.3 send-side record now points at GideonZ/1541ultimate#807 (chunked write, 1472 byte-exact on 3.15-class firmware) rather than the closed #802, and notes the written count is per UCI spec v1.1 §2.10.
