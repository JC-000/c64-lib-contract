...
| `$8A` | `UCI_ERR_LONG_READ` | The SOCKET_READ response header claimed more bytes than were requested **and was not the `$FFFF` no-data sentinel**. **This condition is unobserved; the code is reserved defensively.** `$FFFF` is the firmware's no-data sentinel on both transports (measured: every idle UDP poll across 11 sizes returns header `$FFFF`, `rx_len` 0; the first idle TCP polls of a TLS session, waiting for ServerHello, return `$FFFF` against a 512 B request) and MUST be excluded before any length arithmetic — both fleet adapters independently misfiled it as an over-claim, one for four days as a documented "firmware quirk", the other aborting every handshake on its first idle poll; and the founding `$D000` overrun (c64-wireguard PR #62) is fully explained by 65535 used as a copy count from an idle poll, with no firmware over-claim involved. Disposition if it ever fires, by family: a datagram adapter drops the packet (the firmware discards any excess, GideonZ/1541ultimate#802, so it would be unrecoverable) and emits this code; a stream adapter caps its copy at the request, continues, and records `$8B` instead. The code could not distinguish an oversized peer datagram from a header the device fabricated; do not attribute it to peer MTU policy. c64-wireguard builds before the `$FFFF` exclusion emitted this code on idle polls, meaning "nothing pending". Unmeasured: the header on an oversized UDP datagram (600-1280 B against a 512 B request) — the one place a real over-claim could still be hiding — and the TCP partial-fill case. | terminal (C=1) | c64-wireguard — **in flight**: published `master` still emits this as `$88` (PR #62, 2026-08-20); the move to `$8A` is [c64-wireguard#67](https://github.com/JC-000/c64-wireguard/pull/67), which vacates `$88`. |
