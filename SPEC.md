...
**The basis is the placed span.** Both figures measure the extent the library's `LIB_<X>_`-prefixed segments (§4) occupy in a real link — including alignment fill **internal** to them — **not** the sum of member object sizes. An object-size sum is exact only where no segment takes fragments from two objects *and* every inter-segment gap is charged separately: neither is guaranteed, both stop holding silently on a file split or a translation-unit reorder, and a figure derived from the sum penalises a library that page-aligns a segment to hold a constant-time invariant.

**A figure is the sum of its segments' placed spans.** Gaps *between* two of the library's own segments are placement, not footprint: the gap's size is `(-previous_end) mod alignment`, so it is set by where the consumer put the preceding segment and it disappears if the consumer separates them. Charge what is inside each segment; charge nothing between them. The gap also depends on the *order* the consumer places them in, so a contiguous-extent reading would make the figure a function of the consumer's cfg — the same objection that excludes pre-segment fill. Reordering does not remove the padding, it **relocates** it: the alignment travels with the segment, so putting the aligned segment first moves its pad ahead of it, where the next paragraph excludes it for the same reason. Under either order the bytes exist and neither order makes them the library's. **A consumer reserving one contiguous region for the library must therefore add that padding itself**, computed from the alignments §4 requires the library to declare. A library's own example cfg, which places its segments adjacently, is where that addition is easiest to forget — and the shortfall is silent.

**Padding before a segment is the consumer's, and MUST NOT be charged.** A segment's own alignment against wherever the consumer places it is a function of the previous tenant's end address, so two consumers linking the identical archive pay different amounts and one consumer pays differently after moving an unrelated segment of its own. Charging it worst-case would inflate every aligned segment's declared figure by up to `align - 1` and double-count for any consumer that computes its own layout. The alignment requirement that produces it is already declarable under §4, from which a consumer derives the exact pad for its own map.

**This governs the basis, not the scope.** Which segments a figure covers is unchanged: a library counting a runtime-written window it reserves in the consumer's address space, and one excluding `bss` it does not, are both measuring on the basis this clause fixes.
...
**Member isolation.** ld65 links whole archive members. A symbol a consumer may displace — suppress under `LIB_NO_BARE_EXPORTS`, or define itself under `APP_OWNED` (§8.0) — MUST live in a translation unit that exports nothing else a consumer may import — other displaceable names included, their own prefixed counterparts excepted — and defines nothing else the library's own code references. Otherwise the member arrives uninvited and its displaceable names collide — with the consumer's own definitions, or with the identical bare name a sibling library exports — and the consumer can repair neither: member surgery is banned above.
