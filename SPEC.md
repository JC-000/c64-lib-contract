...
**A figure is the sum of its segments' placed spans.** Gaps *between* two of the library's own segments are placement, not footprint: the gap's size is `(-previous_end) mod alignment`, so it is set by where the consumer put the preceding segment and it disappears if the consumer separates them. Charge what is inside each segment; charge nothing between them. The gap also depends on the *order* the consumer places them in — put the aligned segment first and it vanishes — so a contiguous-extent reading would make the figure a function of the consumer's cfg, which is the same objection that excludes pre-segment fill. **A consumer reserving one contiguous region for the library must therefore add that padding itself**, computed from the alignments §4 requires the library to declare. A library's own example cfg, which places its segments adjacently, is where that addition is easiest to forget — and the shortfall is silent.
