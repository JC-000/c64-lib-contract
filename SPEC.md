...
| `$8A` | `UCI_ERR_LONG_READ` | SOCKET_READ reported more bytes than the adapter requested. (Measured on the TCP path, fw 3.14d, one github.com session: a 512 B request with ~690 B queued answered `$FFFF`; a 22 B request that delivered 22 B answered `$0016`. The second point does not discriminate "delivered" from "available" — request, header and delivered coincide — and the partial-fill case (request N, 0 < queued < N) is unmeasured, so no claim is made about what the header counts; a consumer's copy bound remains safe only on the assumption, so far unrefuted, that the header never understates delivered bytes. c64-wireguard observed `$FFFF` for a 1500 B UDP request; what the UDP header reads for a 600-1280 B datagram against a 512 B request is unmeasured. The row states what the adapter observes and does, not how the device encodes it.) **Disposition depends on the transport, and the code means the datagram case only**: on a byte stream the excess is still queued in the firmware, so the adapter caps its copy at the request and nothing is lost — no error, this code is never emitted; on a datagram transport the firmware discards the excess, the datagram is unrecoverable, and the adapter drops it — that is this code. A correctly MTU-pinned peer never produces it, so its occurrence reports a policy violation upstream observed through a device sentinel, not a device fault. | terminal (C=1) | c64-wireguard — **in flight**: published `master` still emits this as `$88` (PR #62, 2026-08-20); the move to `$8A` is [c64-wireguard#67](https://github.com/JC-000/c64-wireguard/pull/67), which vacates `$88`. c64-https emits `$8A` with this meaning from [c64-https#142](https://github.com/JC-000/c64-https/pull/142). |
| `$8B` | `UCI_ERR_BAD_READ_HDR` | SOCKET_READ's header exceeded the request and was not the `$FFFF` sentinel — a value the firmware has no documented mode for. Best-effort breadcrumb (§13.1 short-write pattern): the adapter still caps its copy at the request and returns C=0; the code is left in `net_last_error` so a post-mortem can tell an undocumented header from a normal read. Distinct from `$8A` on purpose: `$8A` is a disposition (datagram dropped), this is an observation. | **advisory** (C=0) | c64-https — **in flight** ([c64-https#140](https://github.com/JC-000/c64-https/issues/140)) |

The `$8A` row is worded by disposition rather than by transport on purpose: the same device event is recoverable on a stream and terminal on a datagram, and a table that said only "claimed more than requested" was about to give the byte two operational meanings in two UCI-family adapters — the `$88` failure one row down, caught by the cross-repo view a second time inside one release cycle. The table earns its keep by forcing that question before a byte is emitted. The ip65 codes name the adapter entry point that failed rather than a driver cause, because the ip65 driver reports only a carry. Not every backend emits every code in its family's table (`c64-https`'s UCI adapter reserves `$8A` without emitting it yet); a consumer decoding the byte uses the table, not the emitting adapter's header.
