...
**What the figures are measured over.** Both count the `LIB_<X>_`-prefixed segments (§4) the archive contributes: `RESIDENT_BYTES` those a consumer must keep resident, `COLD_BYTES` those it may reclaim after init. **The basis is the placed span** — the extent those segments occupy in a real link, including the fill `ld65` inserts between and within them — **not the sum of member object sizes.** An object-size sum is exact only where no segment takes fragments from two objects *and* every inter-segment gap is charged separately; neither is guaranteed, both stop holding silently on a file split, and a library that satisfies them today gets the right number by a property no clause requires. Which prefixed segments fall on each side MUST be stated beside the equates: a consumer reading the archive cannot otherwise tell, and a figure whose composition is unstated is checkable only by its author.

**How much round-up is permitted.** The declared figure MUST upper-bound every configuration that archive ships in, and MUST exceed the largest of them by no more than 1024 bytes or 10%, whichever is greater. Two consequences are the point of the bound. A library must model every switch that moves the footprint, including one that *swaps* one body for another rather than removing one — a subtractive gate guarantees the direction, a swap does not, and a declaration that covers the alternatives only because today's default happens to be the largest is safe by coincidence. And unearned slack is a defect even though it is in the safe direction: the figure exists to be bound to a budget, so slack spends a consumer's budget on nothing and can fail a `<=` fit assert that should have passed.
